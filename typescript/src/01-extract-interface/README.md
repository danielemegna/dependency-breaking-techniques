...
2. List only the methods your client actually calls on it.
3. Declare a new interface with exactly those methods (e.g. `PaymentGateway`).
4. Make the concrete class implement the new interface. (`implements PaymentGateway`)
5. Change the client to refer to the interface in its field/parameter types.
6. In your test, write a small class implementing the interface and inject it.
7. Run the tests.

## This exercise

`OrderProcessor` (`order-processor.ts`) charges an order through a
`StripePaymentGateway` (`stripe-payment-gateway.ts`). The processor names the
**concrete** gateway type, so the test in `order-processor.test.ts` is forced to
construct a real Stripe gateway — which immediately tries to make an HTTP call
and throws.
