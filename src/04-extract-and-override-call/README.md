...
call**, not an object you hold a reference to. You can't inject a free function
...
- A method makes a **direct call** to a global function, a `static` method, or a
  singleton, and
...
1. Identify the offending call (here: `await sendEmail(to, subject, body)`).
2. Create a new `protected` method on the class whose body is exactly that call,
   taking the same arguments (e.g. `protected dispatchEmail(to, subject, body)`).
3. Replace the inline call with a call to your new method.
4. Confirm behaviour is unchanged — production still calls the real transport.
5. In the test, define a subclass that **overrides** the protected method to
   record its arguments (or assert on them) instead of performing the real call.
6. Drive the test through an instance of the subclass.
7. Run the tests.

> TypeScript note: `protected` members are accessible to subclasses, which is
> all this technique needs. Mark overrides with the `override` keyword (this
> project enables `noImplicitOverride`).

## This exercise

`OrderConfirmationService.confirm()` (`order-confirmation-service.ts`) builds a
confirmation email and sends it by calling the module-level `sendEmail` function
directly. The test in `order-confirmation-service.test.ts` wants to assert who
the email went to and what its subject was — but the real transport throws, and
there is no seam to observe the call.
...
- A bare global call cannot be observed or stubbed.
